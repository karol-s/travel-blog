...
# ADD AND CONFIGURE FLASK LOGIN MANAGER
...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=form.email.data).first():
            flash(f'User with email {form.email.data} already exist.')
            return redirect(url_for("login"))   
        
        hash_and_salted_password = generate_password_hash(form.password.data, 'pbkdf2:sha256', 8)
